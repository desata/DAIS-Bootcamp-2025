...
namespace OfficeResourcesReservationSystem.Repository.Base
{
    public abstract class BaseRepository<TObj>
    {
        ...
        protected virtual async Task<TObj> RetrieveAsync(string idDbFieldName, int idDbValue)
        {
            ...
            if (reader.Read())
            {
                ...
                if (reader.Read())
                {
                    throw new Exception($"More than one record found for the same ID");
                }
